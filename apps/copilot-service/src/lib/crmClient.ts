...
export type { TicketSnapshot, TicketListItem };
...
// CRM API response includes extra fields (id, createdAt, attachments) beyond
// the shared TicketSnapshot shape, so we use a permissive schema for parsing
// and transform to the clean snapshot type.
...
function toTicketSnapshot(ticket: z.infer<typeof TicketResponseSchema>): TicketSnapshot {
  return {
    id: ticket.id,
    title: ticket.title,
...
export async function getTicket(id: string): Promise<CRMResult<TicketSnapshot>> {
  const result = await crmFetch(`/tickets/${id}`, TicketResponseSchema);
  if (result.ok) {
